...
class LinkedList:
    ...
    def generate_dot(self, highlight_ids: List[str] = None) -> str:
        """
        Renders each node as a two-cell record:
          ┌────────┬──────────┐
          │  data  │  next *  │
          └────────┴──────────┘
        Arrows go FROM the 'next' port of one node
        TO the 'data' port of the next node.
        """
        ...
        lines = [
            ...
            '    rankdir=LR;',
            '    bgcolor="#0d0d0d";',
            '    graph [pad="0.5", nodesep="0.7", ranksep="1.0"];',
            '    node [fontname="Courier Bold", fontsize=14, shape=record];',
            '    edge [fontname="Courier", fontsize=11];',
        ]

        # ── HEAD pointer node ──
        if self.head:
            head_addr = self.head.address
            lines.append(
                f'    "HEAD" ['
                f'label="HEAD\\n{head_addr}", shape=plaintext, '
                f'fontcolor="#ffcc00", fontname="Courier Bold", fontsize=14'
                f'];'
            )
            lines.append(
                f'    "HEAD" -> "{self.head.id}":data:w '
                f'[color="#ffcc00", penwidth=2.5, arrowsize=1.3, '
                f'style=bold];'
            )
        else:
            ...
            lines.append(
                ...
                'fontcolor="#ffcc00", fontname="Courier Bold", fontsize=14'
                '];'
            )
            lines.append(
                '    "HEAD_NULL" ['
                'label="NULL", shape=plaintext, '
                'fontcolor="#666666", fontname="Courier Bold", fontsize=13'
                '];'
            )
            lines.append(
                '    "HEAD" -> "HEAD_NULL" '
                '[color="#ffcc00", penwidth=2.0, arrowsize=1.2, style=dashed];'
            )

        cur = self.head
        while cur:
            highlighted = cur.id in highlight_ids
            ...
            next_label = cur.next.address if cur.next else "NULL"

            # Graphviz HTML-like record label:
            # <data|next> — two cells with named ports
            label = (
                f'{{<data> {cur.value}\\n{cur.address} '
                f'| <next> {next_label}}}'
            )
        ...
        # ── Edges: from <next> port → <data> port of next node ──
        cur = self.head
        while cur:
            if cur.next:
                lines.append(
                    f'    "{cur.id}":next:e -> "{cur.next.id}":data:w '
                ...
                lines.append(
                    f'    "{null_id}" ['
                    f'label="NULL", shape=plaintext, '
                    f'fontcolor="#666666", fontname="Courier Bold", fontsize=13'
                    f'];'
                )
                lines.append(
                    f'    "{cur.id}":next:e -> "{null_id}" '
...
st.caption("Interactive visualizer   |  Step-by-step explanation")
